...
const serviceRoleKey = 'eyJhbGciOiJIUzI1NiIsInR5cCI6IkpXVCJ9.eyJpc3MiOiJzdXBhYmFzZSIsInJlZiI6ImdqZGt1eXp1anZwbXBqZXl2cXRrIiwicm9sZSI6InNlcnZpY2Vfcm9sZSIsImlhdCI6MTc3MDY0NzQ2OCwiZXhwIjoyMDg2MjIzNDY4fQ.GWgp6pRBidO1mOsxZV_-jLAWw2rRRU56irt49lPOrf0';
...
console.log('🚀 Ensuring ALL 133 students have complete records...\n');
...
async function ensureAllStudents() {
  ...
  for (const student of students) {
    ...
    try {
      ...
      let authUser = allAuthUsers.find(u => u.email === email);
      ...
      if (!authUser) {
        ...
        if (authError) {
          console.log(`❌ ${student.usn}: Auth error - ${authError.message}`);
          stats.errors++;
          continue;
        }
        ...
        userId = authUser.id;
        allAuthUsers.push(authUser); // Add to cache
        stats.createdAuth++;
        console.log(`✅ ${student.usn}: Created auth`);
      } else {
        userId = authUser.id;
      }
      ...
      if (!profile) {
        ...
        if (profileError) {
          console.log(`⚠️  ${student.usn}: Profile error - ${profileError.message}`);
        } else {
          stats.createdProfile++;
          console.log(`✅ ${student.usn}: Created profile`);
        }
      }

      // 3. Check/Create Student Record
      ...
      if (!existingStudent) {
        ...
        if (studentError) {
          console.log(`⚠️  ${student.usn}: Student error - ${studentError.message}`);
        } else {
          stats.createdStudent++;
          console.log(`✅ ${student.usn}: Created student record`);
        }
      } else {
        stats.complete++;
      }
  ...
  console.log(`🆕 Created auth users: ${stats.createdAuth}`);
  console.log(`🆕 Created profiles: ${stats.createdProfile}`);
  console.log(`🆕 Created student records: ${stats.createdStudent}`);
  console.log(`❌ Errors: ${stats.errors}`);
  console.log(`📊 Total students: ${students.length}`);
  console.log('='.repeat(60));
  console.log('\n🎉 All students ensured!');
  console.log('📧 Login: usn@college.edu');
  console.log('🔑 Password: student123\n');
}

ensureAllStudents().catch(console.error).finally(() => process.exit(0));
